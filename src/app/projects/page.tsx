...
import logoCat from '@/images/cat.png'
...
function fetchRepo(){  
  axios.get('https://api.github.com/users/eugenieeeech/repos')
    .then( (res) => {
      ...
      >
        {projects.toReversed().map((project:any) => (
          <Card as="li" key={project.name}>
            ...
            <Card.Description>{project.description?? "Click to visit the Project"}</Card.Description>
            <p className="relative z-10 mt-6 flex text-sm font-medium text-zinc-400 transition group-hover:text-teal-500 dark:text-zinc-200">
              <LinkIcon className="h-6 w-6 flex-none" />
              <span className="ml-2">{project.full_name}</span>
            </p>
          </Card>
        ))}
      </ul>
